...
HTTP-first контуры в `Controller` и `Service` отдельно ограничивают Playwright fallback двумя одновременными загрузками, чтобы уменьшить пиковое потребление памяти.

Поиск сопоставляет `title` и `original_title` с `title`, `original_title` и `alt_titles` результата. При заданном `year` выбираются совпадения с годом в пределах ±1, что позволяет различать одноимённые фильмы и сериалы разных лет.
