...
Одновременно допускается не более двух Playwright-загрузок внутри Controller и не более двух внутри Service, чтобы не создавать лишний пик памяти.

Поиск учитывает `title`, `original_title` и `alt_titles`. Для сериалов дополнительно учитывается год: подходят результаты с отклонением не более одного года, что позволяет различать одноимённые сериалы разных лет.

Общая проверка anti-bot/auth-ответов и нормализация URL вынесены в `Helpers.cs`, чтобы Controller и Service использовали одинаковые правила.
